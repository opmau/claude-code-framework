...
<!-- ═══════════════════════════════════════════════════════════════════════
     ...
     These markers enable the /retro skill to programmatically update
     specific sections without disturbing the rest of the file.
...
- `docs/LINEAR_SNAPSHOT.md` — Read-only cache of Linear issues (auto-generated by `/linear-sync`)
...
- If you discover a second problem while fixing the first, **document it** using `/document-bug` and address it in a separate session
...
5. **DOCUMENT** using `/document-bug` to log the issue in Linear
...
2. **DOCUMENT IT** using `/document-bug` (creates a Linear issue with `bug` label)
3. **CONTINUE** with extraction
4. **FIX LATER** after the migration is complete — use `/fix-issue ENG-NNN` in a dedicated session
...
1. If a bug **blocks** your current task → fix it, document the fix in the commit message with `[ENG-NNN]`
2. If a bug is **unrelated** to your current task → use `/document-bug` to log it in Linear, finish current work first
3. If a test fails → use `/document-bug` to log it in Linear, do NOT attempt on-the-fly fixes (they often introduce new bugs)
4. To investigate complex bugs → use `/diagnose` for structured differential diagnosis, which records a confirmed root cause on the issue
5. To fix a tracked bug → use `/fix-issue ENG-NNN` to pull from Linear and fix in a focused session. It reproduces the failure and agrees the acceptance criteria with you before writing any code — a tracked issue is a lead, not a specification
...
All issue tracking is done in Linear. Local files are read-only caches:

- **Source of truth:** Linear (issues, bugs, features, tasks)
- **Local snapshot:** `docs/LINEAR_SNAPSHOT.md` — auto-generated by `/linear-sync`
- **Bug workflow:** `/document-bug` → `/diagnose` (unless the cause is already
  confirmed) → `/fix-issue ENG-NNN` → `/linear-update`. The middle step is
  conditional, not optional: an issue carries its root cause as `UNCONFIRMED` or
  `CONFIRMED`, and `/fix-issue` refuses to fix the former
- **CLI reference:** See `.claude/rules/linear-workflow.md` for full command reference
...
| *"/session-mode feature"* | Lock session to adding new functionality |
| *"/diagnose [symptom]"* | Investigate before fixing — structured differential diagnosis |
| *"/document-bug [description]"* | Log a bug in Linear without fixing it — enforces separation |
| *"/fix-issue ENG-NNN"* | Pick a tracked bug from Linear and fix it in a focused session |
...
```
1. Set session mode: /session-mode [debug|refactor|feature]
...
7. Post-session: any CLAUDE.md updates needed? Any bugs to log with /document-bug?
